  ...
  return (
    <h2 className="flex flex-wrap items-stretch gap-2 text-xl font-semibold">
      {items.map((item, index) => (
        <Fragment key={index}>
          {typeof item === 'string' && <span className="text-stone-800">{capitalize(item)}</span>}
          {typeof item !== 'string' && (
            <Link to={item.link}>
              <UnderLineOnHover>
                <span className="text-teal-600 transition-all duration-150 hover:text-teal-600">{capitalize(item.label)}</span>
              </UnderLineOnHover>
            </Link>
          )}
