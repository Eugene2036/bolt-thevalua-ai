...
import type { CalculatorRowItemSchema, NewMutateConstructionSchema } from '~/models/construction.schema';
...
import { useNewCalculatorData } from '~/hooks/useNewCalculatorData';
import { useNewMutateCalculator } from '~/hooks/useNewMutateCalculator';
...
interface Props {
  plotId: string;
  grcId: string | undefined;
  isBull: boolean;
  isAdmin: boolean;
  recordProps: GrcProps | InsuranceProps;
}
export function NewCalculator(input: Props) {
  const { plotId, grcId, isBull, isAdmin, recordProps } = input;

  const query = useNewCalculatorData(plotId, grcId, recordProps, undefined, isBull);

  const { mutate, isMutating: isProcessing } = useNewMutateCalculator(recordProps.recordType === 'insurance');

  const property = query.data?.grc.constructionProp;

  const getNameProp = createGetNameProp<z.infer<typeof NewMutateConstructionSchema>>();
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    console.log('runz');
    const grcId = query.data?.grc?.id;
    if (grcId) {
      const response = await mutate(grcId, {
        plotId,
        isBull,
        floorArea,
        verandaFloorArea,
      ...
      if (typeof response.error === 'string') {
        console.log('___', JSON.stringify(response.error), response.error);
  ...
  const sheets = useCalculatorSheets(isBull, recordProps.recordType === 'insurance');
  ...
  async function handleKindChange(event: ChangeEvent<HTMLSelectElement>) {
    ...
    const latestGrcId = query.data.grc.id;
    await query.refetch(plotId, latestGrcId, recordProps, newKind);
  }
  ...
  return (
    <div style={{ zIndex: 1000 }} className="sticky top-0  pt-[90px] flex flex-col items-stretch bg-transparent border-b border-b-stone-200 shadow-sm print:hidden">
      <div className={twMerge('flex flex-col items-stretch px-2 relative ', query.isLoading && 'animate-pulse')} style={{ zIndex: 1100 }}>
        ...
        <form onSubmit={handleSubmit} className="grow flex flex-col items-stretch overflow-auto gap-2">
          <ActionContextProvider updateState={updateState} isSubmitting={isProcessing}>
            <input type="hidden" {...getNameProp('plotId')} value={input.plotId} />
            <input type="hidden" {...getNameProp('isBull')} value={isBull ? 'true' : ''} />
            ...
            <table className="mb-24">
              ...
              <tfoot>
                ...
                <tr>
                  ...
                  <TableCell className="text-end bg-stone-100 font-bold">
                    {formatAmount(costPerSqMTypical)}
                  </TableCell>
                  <TableCell className="text-end font-normal">
                    {formatAmount(
                      getObso(costPerSqMTypical, costPerSqMQuality),
                    )}
                    %
                  </TableCell>
                  <TableCell className="text-end bg-stone-100 font-bold">
                    {formatAmount(costPerSqMQuality)}
                  </TableCell>
                </tr>
              )} */}
                <tr>
              ...
              <TableCell></TableCell>
              <TableCell></TableCell>
              <TableCell className="items-center"></TableCell>
              <TableCell className="text-end bg-stone-100 font-bold"></TableCell>
              <TableCell className="text-end font-bold"></TableCell>
              <TableCell className="text-end bg-stone-100 font-bold"></TableCell>
            </tr>
            <tr>
              <TableCell></TableCell>
              ...
              <TableCell className="items-center">
                Total Building Estimate (Excl VAT)
              </TableCell>
              <TableCell className="text-end bg-stone-100 font-bold">
                {formatAmount(totalTypicalEstimate)}
              </TableCell>
              <TableCell className="text-end font-bold">Obsolescence</TableCell>
              <TableCell className="text-end bg-stone-100 font-bold">
                {formatAmount(totalQualityEstimate)}
              </TableCell>
            </tr>
            <tr>
              <TableCell></TableCell>
              <TableCell></TableCell>
              <TableCell className="items-center">Cost/M2 (Excl VAT)</TableCell>
              <TableCell className="text-end bg-stone-100 font-bold">
                {formatAmount(costPerSqMTypical)}
              </TableCell>
              <TableCell className="text-end font-normal">
                {formatAmount(getObso(costPerSqMTypical, costPerSqMQuality))}%
              </TableCell>
              <TableCell className="text-end bg-stone-100 font-bold">
                {formatAmount(costPerSqMQuality)}
              </TableCell>
...
function ListItem(props: {
  ...
  // console.log('element', element, 'multiplier id', multiplierIdentifier);
