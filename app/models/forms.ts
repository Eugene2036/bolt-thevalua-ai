...
export type FormFields<K extends FormFieldKey = string> = Partial<Record<K, string>>;
export type FieldErrors<K extends FormFieldKey = string> = Partial<Record<K, string[] | undefined>>;
...
export function hasFormError(data: unknown): data is z.infer<typeof FormErrorSchema> {
...
export function hasFieldErrors(data: unknown): data is z.infer<typeof FieldErrorsSchema> {
...
export function hasErrorMessage(data: unknown): data is z.infer<typeof WithErrMsgSchema> {
  return WithErrMsgSchema.safeParse(data).success;
}

export function objKeysToArr<T extends Record<string, any>>(data: T) {
  return Object.keys(data) as Array<T[keyof T]>;
}

export function objValuesToArr<T extends Record<string, any>>(data: T) {
...
export const FormActionSchema = z.object({
  _action_type: z.enum([FormLiteral.Comment, FormLiteral.TogglePostLike, FormLiteral.ToggleCommentLike]),
});
