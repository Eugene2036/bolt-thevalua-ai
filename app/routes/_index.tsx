...
export async function loader({ request }: LoaderArgs) {
  const [propertyTypes, roofTypes, tenants, plotsData] = await Promise.all([
  ...
  return json({ propertyTypes, roofTypes, tenants, plotsData, numRes, numComm, numValuers });
}
...
export default function Index() {
  const { propertyTypes, roofTypes, tenants, plotsData, numRes, numComm, numValuers } = useLoaderData<typeof loader>();
  ...
  const relevantProperties = loaderData.propertyTypes || loaderData.roofTypes || loaderData.plotsData || loaderData.tenants;

  // const relevantProperties = loaderData.plotsData;

  console.log("Loader Data:", relevantProperties)

  return (
    <main className="m-0 p-0 h-full">
      {/* INITIALIZE CHATBOT WITH DATA ARRAY FROM THE LOADER FUNCTION */}
      {/* <DrawerExample aiContext={({ properties: relevantProperties }).properties.toString()} /> */}
      <Navbar />
      ...
      <section
        id="services"
        className="pb-8 lg:pb-[80px] lg:pt-[50px] container m-auto"
      >
        <h2 className="flex justify-center animate-fade-right pt-5 mb-5 mt-2 text-3xl font-bold leading-tight text-dark text-black sm:text-[40px] sm:leading-[1.2]">
          Property Valuation
        </h2>
        <p className="flex justify-center mb-10 text-base leading-relaxed text-body-color dark:text-dark-6">
