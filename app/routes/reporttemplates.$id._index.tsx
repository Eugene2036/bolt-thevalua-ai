...
export async function loader({ request, params }: LoaderArgs) {
    ...
    return json({ repTemplates, companies, id });
}
...
export default function ReportTemplateIndexPage() {
    ...
    const { repTemplates, companies, id } = useLoaderData<typeof loader>();
    const compID = id;
    ...
    return (
        <div className="flex flex-col items-center justify-start h-full gap-6 py-8">
            <span className="text-xl font-semibold">List of Templates for {companies?.CompanyName}</span>
            <div className="flex flex-col items-stretch min-w-[80%] gap-4">
                <div className="flex flex-row items-center">
                    <div className="flex flex-col justify-center items-center px-4">
                        <span className="text-xl font-semibold">{repTemplates.length} report template(s)</span>
                    </div>
                    <div className="grow" />
                ...
                {repTemplates.map((reportTemplate) => (
                    <ReportTemplateListItem Name={reportTemplate.name} key={reportTemplate.id} {...reportTemplate} />
                ))}
                {!repTemplates.length && <EmptyList>No record(s) found</EmptyList>}
            </div>
        </div>
    );
}
...
function ReportTemplateListItem(props: TemplateListItemProps) {
    ...
    return (
        <div hidden={hasSuccess(fetcher.data)} key={id} className="flex flex-row gap-2 p-4 rounded-lg bg-stone-100">
            ...
            <div className="grow" />
            <PrimaryButtonLink className='h-auto' to={AppLinks.EditReportTemplate(id)}>Edit Template</PrimaryButtonLink>
