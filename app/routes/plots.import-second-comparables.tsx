...
import { COMPARABLE_TABLE_COLUMNS, ComparableExcelRowSchema } from '~/models/excel';
...
const Schema = z.object({
  ...
  }, ComparableExcelRowSchema.array()),
});
...
export async function action({ request, params }: ActionArgs) {
  ...
  for (let row of rows) {
    const [plotNumber, location, neighbourhood, use, plotSize, price, year] = row;
    console.log('use', use);
    ...
    await prisma.comparablePlot.create({
      data: {
        ...
        plotExtent: plotSize,
        propertyType: ValuationType.Residential,
        location,
        suburb: neighbourhood.toString(),
        price,
        transactionDate: dayjs().year(year).toDate(),
        titleDeed: '',
        status: '',
      },
    });
...
export default function ImportComparablesPage() {
  ...
  type CustomRow = readonly [number | string, string, string | number, string, number, number, number];
  ...
  const handleFileChange = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const namedColumns = COMPARABLE_TABLE_COLUMNS.map(([columnName, _]) => {
      ...
      const parsedRows = orderedRows.map((row) => {
        const result = ComparableExcelRowSchema.safeParse(row);
        if (!result.success) {
          console.log(row.map((el) => [el, typeof el]));
          console.log(result.error.flatten());
          return new Error(stringifyZodError(result.error));
        }
        // dayjs(result.data[0]).format('YYYY-MM-DD'),
        return [result.data[0], result.data[1], result.data[2], result.data[3], result.data[4], result.data[5], result.data[6]] as const;
      });
      setRows(parsedRows);
    } catch (error) {
      console.log(error);
        ...
        <table className="table-auto border-collapse text-left">
          <thead className="divide-y rounded border border-stone-200">
            <tr className="divide-x border border-stone-200">
              {COMPARABLE_TABLE_COLUMNS.map(([col, _], index) => (
            ...
            <tbody className="text-base">
              {[...Array(10).keys()].map((key) => (
                <tr key={key} className="divide-x divide-stone-200 border border-stone-200">
                  {COMPARABLE_TABLE_COLUMNS.map((_, index) => (
