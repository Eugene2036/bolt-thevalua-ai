...
import { badRequest, processBadRequest, roundDown } from '~/models/core.validations';
...
export async function action({ request, params }: ActionArgs) {
  ...
  try {
    ...
    const result = AddInsuranceSchema.safeParse(fields);
    if (!result.success) {
    ...
    await prisma.$transaction(async (tx) => {
      const record = await tx.insurance.create({
        data: result.data,
      });
