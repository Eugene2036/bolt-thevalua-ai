...
import { COUNCIL_PLOT_TABLE_COLUMNS_3, CouncilPlotExcelRowSchema3 } from '~/models/excel';
...
const Schema = z.object({
  ...
  }, CouncilPlotExcelRowSchema3.array()),
});
...
export async function action({ request, params }: ActionArgs) {
  ...
  for (let row of rows) {
    ...
    console.log(plotSize, inspectionDate, structure, location);

    const plot = plots.find((p) => p.plotNumber === plotNumber.toString());

    if (plot) {
      // if (plot.propertyLocation) {
      //   continue;
      // }

      console.log('plot found >>>', plot.valuationType);
      updates += 1;

      // console.log('Upadting land rate...');
      const storedValues = await prisma.storedValue.findMany({
        where: { plotId: plot.id, identifier: StoredValueId.LandRate },
      });
      if (storedValues.length) {
        await Promise.all(
          storedValues
            .filter((v) => Number(v.value) !== landRate)
            .map((value) => {
              return prisma.storedValue.update({
                where: { id: value.id },
                data: { value: landRate },
              });
            }),
        ...
        await prisma.storedValue.create({
          data: {
            identifier: StoredValueId.LandRate,
            value: landRate,
            plotId: plot.id,
          },
        });
      ...
      // console.log('Upadting extenstion...');
      if (plot.propertyLocation !== extension) {
        await prisma.plot.update({
          where: { id: plot.id },
          data: {
            propertyLocation: extension,
            // plotExtent: plotSize,
            // inspectionDate,
            // address: location,
            // valuationType: ValuationType.Commercial,
            // address: location + ' ' + neighbourhood,
          },
        });
        console.log('Updated extenstion', rows.indexOf(row), 'of', rows.length);
      }
      continue;
    }
    ...
    //     return temp + 1;
    //   }
    //   const lastPlotId = plots.length
    //     ? plots.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0]
    //         .propertyId
    //     : 0;
    //   return lastPlotId + 1;
    // })();
  ...
  const handleFileChange = useCallback(
    async (event: ChangeEvent<HTMLInputElement>) => {
      try {
        ...
        const namedColumns = COUNCIL_PLOT_TABLE_COLUMNS_3.map(([columnName, _]) => {
        ...
        const parsedRows = orderedRows.map((row) => {
          const result = CouncilPlotExcelRowSchema3.safeParse(row);
          if (!result.success) {
          ...
          return [
            ...
            // dayjs(result.data[2]).format('YYYY-MM-DD'),
            result.data[3],
            result.data[4],
            result.data[5],
            result.data[6],
        ...
        <table className="table-auto border-collapse text-left">
          <thead className="divide-y rounded border border-stone-200">
            <tr className="divide-x border border-stone-200">
              {COUNCIL_PLOT_TABLE_COLUMNS_3.map(([col, _], index) => (
            ...
            <tbody className="text-base">
              {[...Array(10).keys()].map((key) => (
                <tr key={key} className="divide-x divide-stone-200 border border-stone-200">
                  {COUNCIL_PLOT_TABLE_COLUMNS_3.map((_, index) => (
