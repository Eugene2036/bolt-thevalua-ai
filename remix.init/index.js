...
const cleanupCypressFiles = ({ fileEntries, isTypeScript, packageManager }) =>
  fileEntries.flatMap(([filePath, content]) => {
    let newContent = content.replace(new RegExp('npx ts-node', 'g'), isTypeScript ? `${packageManager.exec} ts-node` : 'node');

    if (!isTypeScript) {
      newContent = newContent.replace(new RegExp('create-user.ts', 'g'), 'create-user.js').replace(new RegExp('delete-user.ts', 'g'), 'delete-user.js');
    }
...
const cleanupDeployWorkflow = (deployWorkflow, deployWorkflowPath) => {
  ...
  deployWorkflow.jobs.deploy.needs = deployWorkflow.jobs.deploy.needs.filter((need) => need !== 'typecheck');
  ...
  ({
    ...
    pnpm: () => {
      ...
      return {
        ...
        run: (script, args) => (includeDoubleDashBeforeArgs ? `pnpm run ${script} ${args ? `-- ${args}` : ''}` : `pnpm run ${script} ${args || ''}`),
      };
    },
    yarn: () => ({
      exec: 'yarn',
...
const removeUnusedDependencies = (dependencies, unusedDependencies) => Object.fromEntries(Object.entries(dependencies).filter(([key]) => !unusedDependencies.includes(key)));

const updatePackageJson = ({ APP_NAME, isTypeScript, packageJson }) => {
  const {
    devDependencies,
    prisma: { seed: prismaSeed, ...prisma },
    scripts: { 'format:repo': _repoFormatScript, 'lint:repo': _repoLintScript, typecheck, validate, ...scripts },
  } = packageJson.content;
  ...
  packageJson.update({
    ...
    devDependencies: removeUnusedDependencies(
      ...
      ['eslint-plugin-markdown', 'eslint-plugin-prefer-let'].concat(isTypeScript ? [] : ['ts-node']),
    ),
    prisma: isTypeScript
    ...
    scripts: isTypeScript ? { ...scripts, typecheck, validate } : { ...scripts, validate: validate.replace(' typecheck', '') },
  });
};
...
const main = async ({ isTypeScript, packageManager, rootDirectory }) => {
  ...
  const [prodContent, readme, env, dockerfile, cypressCommands, createUserCommand, deleteUserCommand, deployWorkflow, vitestConfig, packageJson] = await Promise.all([
...
`;
  ...
  const newDockerfile = pm.lockfile ? dockerfile.replace(new RegExp(escapeRegExp('ADD package.json'), 'g'), `ADD package.json ${pm.lockfile}`) : dockerfile;
